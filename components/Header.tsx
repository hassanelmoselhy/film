...
const Header = () => {
  ...
  const handleSignIn = () => {
    router.push('/local'); // التوجيه إلى الصفحة المطلوبة
  };
  ...
  return (
    <header className='relative flex items-center justify-between container py-4 text-black dark:text-white' >
     ...
     <SignedOut>
      <SignInButton
        className='bg-red-45 px-3 py-2 font-medium text-white rounded-lg hover:bg-red-60'
        onClick={handleSignIn} // إضافة دالة التوجيه هنا
      >
        {t('signin')}
      </SignInButton>
    </SignedOut>
